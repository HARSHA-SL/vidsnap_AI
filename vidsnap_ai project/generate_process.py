...
def text_to_audio(folder):
    print("TTA - ", folder)
    with open(f"user_uplodes/{folder}/desc.txt") as f:
        text = f.read()
        print(text,folder)
    ...
    # uncomment this to convert your text to audio because it consumes tokens for elevenlabs app .

def create_reel(folder):
    command = (
        f'ffmpeg -f concat -safe 0 -i user_uplodes/{folder}/input.txt '
        f'-i user_uplodes/{folder}/audio.mp3 '
...
if __name__ == "__main__":
    while True:
        ...
        folders = os.listdir("user_uplodes") 
